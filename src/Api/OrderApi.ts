...
const price = async (filter: string, db?: string): Promise<number> => {
   
    const { data } = await req.httpAuth("").get("/tenant/order/associate/price" + filter + (db ?? ApiConfig.db));
    return data
}
