import { useEffect, useState } from "react";
import { useGetAllProductsByFilterService, useGetAllProductsService } from "../../Api/Services";
import { HomePageSection } from "../../Types/ThemeSetting";
import ProductCard from "../ProductCard";
import { ActionIcon, Button, Tooltip } from "rizzui";
import { AiOutlinePlus } from "react-icons/ai";
type ProductsTotal = {
  data: Product[],
  total: number
}

export default function ProductsSctions({ data }: { data: HomePageSection }) {
  const limit = data.limit ?? 6
  const [page, setPage] = useState(1)




  const [dt, setDt] = useState<ProductsTotal>({
    data: [],
    total: 0
  })
  const { isLoading, mutate } = useGetAllProductsService(limit, page,
    {
      revalidateOnMount: true,
      onSuccess: (res: ProductsResponse) => {
        if (res.data) {
          console.log(res.data)
          setDt({
            data: [...dt.data, ...res.data],
            total: res.totalCount
          })

        }
      },
    })

  useEffect(() => {
    mutate()
  }, [page])
  return (
    <div className="my-4">
      <h1 className="text-center font-bold text-2xl">{data.title}</h1>
      <p className="text-center">{data.subTitle}</p>
      <div className="grid grid-cols-5 gap-4 mt-4 max-sm:grid-cols-2 max-sm:gap-1">

        {
          dt?.data.map((el, k) => {
            return <ProductCard data={el} key={k} />
          })
        }
      </div>
      <div className="mt-4">
        {
          (!isLoading && Math.ceil(dt.total / limit) > page) && <div className="flex justify-center ">
            <Tooltip rounded="lg" content={"show more"}>


              <ActionIcon rounded="full" variant="outline" onClick={() => {
                if (!isLoading) {
                  setPage(page + 1)
                }
              }} className=" !h-12 !w-12" size="lg" color="primary">
                <AiOutlinePlus className="w-5 h-5" />
              </ActionIcon>
            </Tooltip>
            {/* <Button onClick={() => {
              if (!isLoading) {
                setPage(page + 1)
              }
            }} style={{...data.l}}>
            {data.Button?.title}
          </Button> */}
          </div>
        }
        {isLoading && <div className="flex justify-center">
          <svg aria-hidden="true" role="status" className="inline w-16 h-16 me-3 text-primary animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB" />
            <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor" />
          </svg>

        </div>}
      </div>

    </div>
  )
}
